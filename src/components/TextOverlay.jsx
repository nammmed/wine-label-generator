...
const TextOverlay = ({ image, textData }) => {
    ...
    const drawText = async () => {
        ...
        img.onload = () => {
            ...
            // Алкоголь
            const alcoholText = `АЛК. ${textData.alcohol}%`;
            ctx.textAlign = 'right';
            drawTextWithBackground(
                ctx,
                alcoholText,
                centerX - (img.width * 0.1),
                img.height * 0.85,
                ctx.measureText(alcoholText).width,
                img.width * 0.035
            );

            // Сахар
            const sugarText = `САХАР ${textData.sugar} г/л`;
            ctx.textAlign = 'left';
            drawTextWithBackground(
                ctx,
                sugarText,
                centerX + (img.width * 0.1),
                img.height * 0.85,
                ctx.measureText(sugarText).width,
                img.width * 0.035
            );
            ...
            grapeLines.forEach((line, index) => {
                const y = img.height * 0.9 + (index * img.width * 0.04);
                const textWidth = ctx.measureText(line).width;
                drawTextWithBackground(
                    ctx,
                    line,
